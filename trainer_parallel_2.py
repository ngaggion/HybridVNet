...
def train_step(model, config, optimizer, queue, j, image, target, lax2ch, lax3ch, lax4ch, target_d1, target_d2, target_d3, target_d4, faces, index_a, index_b):         
    ...
    out, ds = model(image, lax2ch, lax3ch, lax4ch)
    ...
    if w_laplacian > 0:
        ...
        train_w_laplacian_loss = w_laplacian * loss_laplacian.item()
    else:
        loss_laplacian = 0

    if w_shape > 0 and not config['surface']:
        well_shaped_loss = well_shaped_loss(out, index_a, index_b)
        train_w_shape_loss = well_shaped_loss.item()
    else:
        well_shaped_loss = 0

    # Kullback-Leibler divergence term
    kld_loss = -0.5 * torch.mean(torch.mean(1 + model.log_var - models[j].mu ** 2 - model.log_var.exp(), dim=1), dim=0)
    
    loss = outloss + ds_loss + model.kld_weight * kld_loss + w_laplacian * loss_laplacian + w_shape * well_shaped_loss
    ...
    train_kld_loss_avg = models[j].kld_weight * kld_loss.item()
    train_rec_loss_avg = outloss.item()
    train_loss_avg = loss.item()
    
    if w_ds > 0:
        train_ds_loss = ds_loss.item()
    else:
        train_ds_loss = 0
        
    queue.put((train_kld_loss_avg, train_rec_loss_avg, train_loss_avg, train_ds_loss))

    return


def evaluate(queue, model, image, target, lax2ch, lax3ch, lax4ch):
    out, _ = model(image, lax2ch, lax3ch, lax4ch)
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        for sample_batched in train_loader:
            ...
            for j in range(len(models)):
                ...
                queue = Queue()
                queues.append(queue)
                p = Process(target=train_step, args=(models[j], configs[j], optimizer, queue, j, image, target, lax2ch, lax3ch, lax4ch, target_d1, target_d2, target_d3, target_d4, faces, index_a, index_b))
                p.start()
            
            for j in range(len(models)):
                train_kld_loss_avg[j] += queues[j].get()[0]
                train_rec_loss_avg[j] += queues[j].get()[1]
                train_loss_avg[j] += queues[j].get()[2]
                train_ds_loss[j] += queues[j].get()[3]
            ...
            with torch.no_grad():
                ...
                for sample_batched in val_loader:
                    ...
                    for j in range(len(models)):
                        queue = Queue()
                        queues.append(queue)
                        p = Process(target=evaluate, args=(queue, models[j], image, target, lax2ch, lax3ch, lax4ch))
                        p.start()
                    
                    for j in range(len(models)):
                        val_loss_avg[j] += queues[j].get()
                        p.join()
