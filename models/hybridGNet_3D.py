...
class Encoder2D(nn.Module):
    def __init__(self, latents = 16, hw = 7):
...
class HybridGNet3D(nn.Module):
    def __init__(self, config, downsample_matrices, upsample_matrices, adjacency_matrices, do_skip = None):
        ...
        self.lax2ch_encoder = Encoder2D(self.latents2D)
        self.lax3ch_encoder = Encoder2D(self.latents2D)
        self.lax4ch_encoder = Encoder2D(self.latents2D)
        ...
        self.linear_decoder = torch.nn.Linear(self.latents + self.latents2D * 3, self.filters[5] * n_nodes[-1])
    ...
    def forward(self, sax, lax2ch, lax3ch, lax4ch):
        mu, log_var, layers = self.encoder(sax)
        mu2, log_var2 = self.lax2ch_encoder(lax2ch)
        mu3, log_var3 = self.lax3ch_encoder(lax3ch)
        mu4, log_var4 = self.lax4ch_encoder(lax4ch)

        self.mu = torch.concat([mu, mu2, mu3, mu4], axis = 1)
        self.log_var = torch.concat([log_var, log_var2, log_var3, log_var4], axis = 1)
