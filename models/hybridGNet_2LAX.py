...
class HybridGNet3D(nn.Module):
    def __init__(self, config, downsample_matrices, upsample_matrices, adjacency_matrices, do_skip = None):
        ...
        self.lax1_encoder = EncoderConv()
        self.lax2_encoder = EncoderConv()
        ...
        self.linear_decoder = torch.nn.Linear(self.latents + 32, self.filters[5] * n_nodes[-1])
    ...
    def forward(self, sax, lax1,lax2):
        mu, log_var, layers = self.encoder(sax)
        mu2, log_var2 = self.lax1_encoder(lax1)
        mu3, log_var3 = self.lax2_encoder(lax2)

        self.mu = torch.concat([mu, mu2, mu3], axis = 1)
        self.log_var = torch.concat([log_var, log_var2, log_var3], axis = 1)
