...
from models.utils import Pool as Pooling
import torch 
import shutil

def process_subject(subject, out_path, faces, subs, dic):
    subject_id = subject.split("/")[-1]
    
    pooling = Pooling()
    D2 = dic['D'][1]
    D2 = sp.csc_matrix(D2).tocoo()
    D2 = scipy_to_torch_sparse(D2)
    D3 = dic['D'][2]
    D3 = sp.csc_matrix(D3).tocoo()
    D3 = scipy_to_torch_sparse(D3)

    timesteps = load_folder(subject)
    for time in timesteps:
        time_id = time.split("/")[-1].replace(".npy","").replace("fhm_","")
        ...
        m = torch.tensor(mesh).unsqueeze(0)
        m2 = pooling(m, D2)[0]
        m3 = pooling(m2, D3)[0]
        m3 = m3.cpu().numpy()

        mask_seg = get_mask_image(m3, faces, subs, image).transpose(2,0,1)
        ...
        # Move mesh to the save folder under the name mesh.npy
        
        save_folder = save_folder.replace("Masks","Meshes")
        if not os.path.exists(save_folder):
            os.makedirs(save_folder, exist_ok=True)
            
        np.save(os.path.join(save_folder, "mesh.npy"), m3)
...
if __name__ == "__main__":    
    ...
    out_path = "../Predictions/YanSurface/Masks"

    mesh_path = "../Predictions/YanSurface/Results"
    subjects = load_folder(mesh_path)
    
    dic = pkl.load(open("../Backup/Dataset_Creation/part-meshes/unified_downsample.pkl","rb"))
            
    # Create a partial function with the common arguments
    func = partial(process_subject, out_path=out_path, faces=faces, subs=subs, dic=dic)
